...
# ——————————————————————————————
# ١) أول ما يبدأ السكربت
st.set_page_config(page_title="Zone‑Pro with Dashboard", layout="centered")

# ——————————————————————————————
# ٢) صفحة أو تبويب: Calculator أو Dashboard
page = st.sidebar.radio("اختر الصفحة:", ["Calculator", "Dashboard"])

# ——————————————————————————————
# ٣) تهيئة Session State للمقاييس
...
if page == "Calculator":
    ...
    if task == "Zeta Zero γₙ":
        N = st.number_input(
            "N = رقم الصفر الذي تريد حسابه (0–100000)", 
            min_value=0, max_value=100000, value=0, step=1
        )
        do_plot = st.checkbox("أرسم منحنى الأصفار حتى N", value=False)
        ...
        X = st.number_input(
            "X = احسب π(X) حتى (0–10000000)", 
            min_value=0, max_value=10000000, value=0, step=1
        )
        if st.button("احسب"):
            if X > 0:
                piX = primepi(X)
